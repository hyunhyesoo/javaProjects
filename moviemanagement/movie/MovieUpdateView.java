...
public class MovieUpdateView extends JPanel {
    ...
    public MovieUpdateView() {
        ...
        buttonUD = new JButton("영화수정");
        buttonDEL = new JButton("영화삭제");
        panelUD = new JPanel(new GridLayout(4, 4));
        int i;
        for (i = 0; i < labels.length; i++) {
            labels[i] = new JLabel(header[i]);
            panelUD.add(labels[i]);

            if (i < header.length - 1) {
                textFields[i] = new JTextField();
                panelUD.add(textFields[i]);
            } else {
                panelUD.add(ratingCombo);
            }
        }
    ...
    public void setTextField(int rowIndex) {
        ...
        ratingCombo.setSelectedItem(model.getValueAt(rowIndex, 4).toString());
    }
